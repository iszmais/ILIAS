...
class Conductor
{
    ...
    public function loginPageHTML(): string
    {
        ...
        return $this->container->ui()->renderer()->render(array_merge(...array_map(
            ...
            $this->internal->all('showOnLoginPage')
        )));
    }
    ...
    public function logoutText(): string
    {
        ...
        $logout_text = $this->internal->get('logoutText', $id);
    ...
    {
        $key = ilLegalDocumentsAgreementGUI::class === $gui ? 'agreement_form' : 'agreement_public';
    ...
    {
        $filters = $this->internal->all('filterOnlineUsers');
    ...
    {
        return $this->container->refinery()->in()->series(array_values($this->internal->all('constrainInternalMail')));
    }

    public function canUseSoapApi(): Transformation
    {
        return $this->container->refinery()->in()->series(array_values($this->internal->all('useSoapApi')));
    }

    public function afterLogin(): void
    {
        array_map(fn($proc) => $proc(), $this->internal->all('afterLogin'));
    }
    ...
    {
        return new Bundle($this->internal->all('self_registration'));
    }

    public function userManagementFields(ilObjUser $user): array
    {
        return array_reduce(
            $this->internal->all('user_management_fields'),
