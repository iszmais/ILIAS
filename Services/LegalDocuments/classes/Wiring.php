...
class Wiring implements UseSlot
{
    ...
    {
        return $this->addTo('afterLogin', Closure::fromCallable($after_login));
    }
    ...
    public function canWithdraw(WithdrawProcess $withdraw_process): self
    {
        ...
        return $this->addTo('withdraw', $this->slot->id(), $withdraw)
                    ...
                    ->addTo('logoutText', $this->slot->id(), $withdraw_process->showValidatePasswordMessage(...))
                    ->addTo('showOnLoginPage', $this->slot->withdrawalFinished($withdraw_process->withdrawalFinished(...)));
    }

    public function showOnLoginPage(callable $show): self
    {
        return $this->addTo('showOnLoginPage', Closure::fromCallable($show));
    }

    public function hasPublicPage(callable $public_page): self
    {
        return $this->addTo('agreement_public', $this->slot->id(), fn(...$args) => new Ok($public_page(...$args)));
    }
    ...
    public function hasAgreement(Agreement $on_login, ?string $goto_name = null): self
    {
        ...
        $wiring = $this->addTo('agreement_public', $this->slot->id(), fn(...$args) => new Ok($on_login->showAgreement(...$args)))
                       ->addTo('agreement_form', $this->slot->id(), $this->protect($on_login->showAgreementForm(...), $on_login->needsToAgree(...)))
    ...
    {
        return $this->addTo('self_registration', $self_registration);
    }

    public function hasOnlineStatusFilter(callable $only_visible_users): self
    {
        return $this->addTo('filterOnlineUsers', $only_visible_users);
    }

    public function canReadInternalMails(Constraint $constraint): self
    {
        return $this->addTo('constrainInternalMail', $constraint);
    }

    public function canUseSoapApi(Constraint $constraint): self
    {
        return $this->addTo('useSoapApi', $constraint);
    }
    ...
    public function hasDocuments(array $content_as_component = [], array $available_conditions = []): self
    {
        ...
        return $this->addTo('document', $this->slot->id(), $document)
                    ->addTo('writableDocument', $this->slot->id(), $this->slot->document($repository, $available_conditions, $content_as_component));
    }

    public function hasUserManagementFields(callable $field_value): self
    {
        return $this->addTo('user_management_fields', $this->slot->id(), $field_value);
    }
    ...
    {
        return static fn(...$args): Result => $protector() ? new Ok($to_be_protected(...$args)) : new Error('Nope');
    }
