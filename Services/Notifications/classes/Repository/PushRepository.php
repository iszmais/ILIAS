...
class PushRepository
{
    ...
    public function addSubscription(string $endpoint, string $p256dh, string $auth): void
    {
        $this->database->insert(
            'push_subscriptions',
            [
                'endpoint' => [ilDBConstants::T_TEXT, $endpoint],
                'user_id' => [ilDBConstants::T_INTEGER, $this->user->getId()],
                'p256dh' => [ilDBConstants::T_TEXT, $p256dh],
                'auth' => [ilDBConstants::T_TEXT, $auth],
            ]
        );
    }
    ...
    public function getUserSubscriptions(?int $user_id = null): array
    {
        ...
        while ($row = $stmt->fetchAssoc()) {
            $sub = new PushSubscription($row['endpoint']);
            $sub->setP256dh($row['p256dh']);
            $sub->setAuth($row['auth']);
            $result[] = $sub;
        }
